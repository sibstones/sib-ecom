...
export interface UpdateHeaderSettingsDto {
  isActive?: boolean;
  logoType?: 'TEXT' | 'IMAGE' | 'SVG';
  logoText?: string;
  logoImageUrl?: string;
...
export const headerApi = {
  getSettings: () => apiClient.get<{ settings: HeaderSettings }>('/header'),
  getActiveSettings: () => apiClient.get<{ settings: HeaderSettings }>('/header/active'),
  updateSettings: (data: UpdateHeaderSettingsDto) =>
    apiClient.put<{ settings: HeaderSettings }>('/header', data),
  deleteSettings: (id: string) => apiClient.delete<{ message: string }>(`/header/${id}`),
};
