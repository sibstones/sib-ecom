...
export function resolvePaymentInstruction(
  ...
  const instructionEn = typeof config?.instructionEn === 'string' ? config.instructionEn.trim() : '';
  const instructionZh = typeof config?.instructionZh === 'string' ? config.instructionZh.trim() : '';
